...
import (
	...
	"github.com/Gundy-Zhou/component/pkg/util/clock"
	"github.com/Gundy-Zhou/component/pkg/util/runtime"
)
...
// TestWaitForClosesStopCh verifies that after the condition func returns true, WaitFor() closes the stop channel it
// supplies to the WaitFunc.
...
func TestBackoff_Step(t *testing.T) {
	tests := []struct {
		...
		{
			initial: &Backoff{Duration: time.Second, Steps: 0},
			want:    []time.Duration{time.Second, time.Second, time.Second},
		},
		{
			initial: &Backoff{Duration: time.Second, Steps: 1},
			want:    []time.Duration{time.Second, time.Second, time.Second},
		},
		{
			initial: &Backoff{Duration: time.Second, Factor: 1.0, Steps: 1},
			want:    []time.Duration{time.Second, time.Second, time.Second},
		},
		{
			initial: &Backoff{Duration: time.Second, Factor: 2, Steps: 3},
			want:    []time.Duration{1 * time.Second, 2 * time.Second, 4 * time.Second},
		},
		{
			initial: &Backoff{Duration: time.Second, Factor: 2, Steps: 3, Cap: 3 * time.Second},
			want:    []time.Duration{1 * time.Second, 2 * time.Second, 3 * time.Second},
		},
		{
			initial: &Backoff{Duration: time.Second, Factor: 2, Steps: 2, Cap: 3 * time.Second, Jitter: 0.5},
			want:    []time.Duration{2 * time.Second, 3 * time.Second, 3 * time.Second},
		},
		{
			initial: &Backoff{Duration: time.Second, Factor: 2, Steps: 6, Jitter: 4},
			want: []time.Duration{
				1 * time.Second,
				2 * time.Second,
				4 * time.Second,
...
func TestExponentialBackoffManagerWithRealClock(t *testing.T) {
	...
	backoffMgr := NewExponentialBackoffManager(
		1*time.Millisecond,
		10*time.Millisecond,
		1*time.Hour,
		2.0,
